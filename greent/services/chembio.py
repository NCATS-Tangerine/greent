...
class ChemBioKS(Service):
    ...
    def graph_get_genes_by_disease (self, disease): #reasoner
        ...
        results = []
        predicate=LabeledID(identifier="RO:0002326", label="contributes_to")
        for r in response:
            node = KNode ("UNIPROT:{0}".format (r['uniprotGene'].split('/')[-1:][0]),  type=node_types.GENE)
    ...
    def graph_get_pathways_by_gene (self, gene): #reasoner        
        ...
        for r in response:
            node = KNode ("KEGG:{0}".format (r['keggPath'].split('/')[-1:][0]), type=node_types.PATHWAY)
    ...
    def graph_drugbank_to_uniprot (self, drugbank):
        ...
        predicate=LabeledID(identifier='SIO:001257', label='chemical to gene association')
        results = []
        for r in response:
            node = KNode ("UNIPROT:{0}".format (r['uniprotGeneID'].split('/')[-1:][0]), type=node_types.GENE)
    ...
    def graph_diseasename_to_uniprot (self, disease):
        ...
        if len(response) > 0: # This is a disease.
            ...
            predicate=LabeledID(identifier='NCIT:R176', label='disease to gene association')
            for r in response:
                chemPmids = r['chemPmids']
                disPmids = r['disPmids']
                pmids = chemPmids + "|" + disPmids
                node = KNode ("UNIPROT:{0}".format (r['uniprotSym'].split('/')[-1:][0]), type=node_types.GENE)
    ...
    def graph_pubchem_to_ncbigene( self, pubchem_node):
        ...
        predicate=LabeledID(identifier='CTD:interacts_with', label='interacts')
        retvals = []
        for r in response:
            props = {}
            ...
            node = KNode( "NCBIGene:{}".format( r['NCBIGene']), type=node_types.GENE)
