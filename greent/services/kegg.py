...
class KEGG(Service):
    ...
    def enzyme_get_chemicals(self,enzyme_node):
        ...
        for reaction_id in reactions:
            ...
            if 'enzyme' in rxn:
                enzyme = KNode(rxn['enzyme'], type=node_types.GENE)
                if len(enzyme_ids.intersection(rxn['reactants'])) > 0:
                    predicate = LabeledID('RO:0002449','negatively regulates, entity to entity')
                    input_identifier = enzyme_ids.intersection(rxn['reactants']).pop()
                elif len(enzyme_ids.intersection(rxn['products'])) > 0:
                    predicate = LabeledID('RO:0002450','positively regulates, entity to entity')
                    input_identifier = enzyme_ids.intersection(rxn['products']).pop()
                else:
                    logger.error(f"Mismatch between query and answer: {rxn} {enzyme_ids}")
                    continue
                edge = self.create_edge(enzyme, enzyme_node, f'kegg.chemical_get_enzyme',  input_identifier, predicate)
                results.append( (edge, enzyme))
        return results
